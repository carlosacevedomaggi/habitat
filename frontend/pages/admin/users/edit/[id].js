// Placeholder for Admin Edit User Page
...
export default function EditUserPage() {
    ...
    fetch(`${API_ROOT}/api/users/${userId}`, { 
      ...
      .then(data => {
        setFormData({
          ...
          email: data.email || '',
          password: '', // Always clear password field on load for security
  ...
  const handleSubmit = async (e) => {
    ...
    if (!dataToUpdate.password) { // If password field is empty, don't send it
    ...
    try {
      ...
      toast.error('An unexpected error occurred updating the user.');
      setError('An unexpected error occurred updating the user.');
    } finally {
      setIsSubmitting(false);
    }
  };
  
  if (!userId) return <AdminLayout title="Edit User"><p className="text-center text-gray-400">Loading user ID...</p></AdminLayout>;
  if (loading) return <AdminLayout title="Edit User"><p className="text-center text-gray-400">Loading user data...</p></AdminLayout>;
  if (error && !formData.username && !loading) return <AdminLayout title="Error"><p className="text-center text-red-500">Error: {error}</p><Link href="/admin/users" className="block text-center text-accent mt-4">Back to Users</Link></AdminLayout>;


  return (
    <AdminLayout title={`Edit User: ${formData.username || ''}`}>
      <Head>
        <title>Edit User {formData.username || userId} - Habitat Admin</title>
      </Head>
      <div className="bg-gray-800 p-6 md:p-8 rounded-lg shadow-lg max-w-lg mx-auto">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-semibold text-accent">Edit User</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label htmlFor="password" className="block text-sm font-medium text-gray-300 mb-1">Password (leave blank to keep current)</label>
            <input type="password" name="password" id="password" value={formData.password} onChange={handleChange} className="w-full input-style" placeholder="New password (optional)"/>
          </div>
          ...
          <div className="flex justify-end space-x-4 pt-4">
            ...
            <button type="submit" disabled={isSubmitting || loading} className="btn-submit-loading">
              ...
              {(isSubmitting || loading) ? 'Saving...' : 'Update User'}
            </button>
          </div>
        </form>
      </div>
