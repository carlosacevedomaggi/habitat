              ...
              <tbody>
                {users.map((user) => (
                  <tr key={user.id} className="border-b border-gray-600 hover:bg-gray-600/50 transition-colors">
                    ...
                    <td className="p-4">{user.is_admin ? <span className="px-2 py-1 text-xs font-semibold text-green-100 bg-green-600 rounded-full">Yes</span> : <span className="px-2 py-1 text-xs font-semibold text-red-100 bg-red-600 rounded-full">No</span>}</td>
                    <td className="p-4">{user.is_editor ? <span className="px-2 py-1 text-xs font-semibold text-green-100 bg-green-600 rounded-full">Yes</span> : <span className="px-2 py-1 text-xs font-semibold text-red-100 bg-red-600 rounded-full">No</span>}</td>
                    <td className="p-4 space-x-2 whitespace-nowrap">
                      <Link href={`/admin/users/edit/${user.id}`}
                        className="text-blue-400 hover:text-blue-300 transition-colors py-1 px-2 rounded hover:bg-blue-500/20">
                        <svg className="w-5 h-5 inline mr-1" fill="currentColor" viewBox="0 0 20 20"><path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z"></path><path fillRule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clipRule="evenodd"></path></svg> Edit
                      </Link>
                      <button 
